...
const APP_URL = import.meta.env.VITE_APP_URL || 'https://jexpay.in';
...
const adminAPI = {
  ...
  logoutAll: (userId) => authFetch(`${API_BASE}/admin/logout-all`, { method: 'POST', body: JSON.stringify({ userId }) }),
  checkLogout: () => authFetch(`${API_BASE}/admin/check-logout`),
  checkTokenVersion: () => authFetch(`${API_BASE}/admin/check-tokenversion`),
  ...
  toggleManagerJTokenRequest: (managerId, enabled) => authFetch(`${API_BASE}/admin/manager/${managerId}/jtoken-request-toggle`, { method: 'PUT', body: JSON.stringify({ managerJTokenRequestEnabled: enabled }) }),
  ...
  createManager: (data) => authFetch(`${API_BASE}/direct-create-manager`, { method: 'POST', body: JSON.stringify(data) }),
