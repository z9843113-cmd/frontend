...
const BuyJToken = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const [walletRes, requestsRes, userUpiRes, settingsRes, jtokenAppsRes, paymentAppsRes, profileRes] = await Promise.all([
          ...
          <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] rounded-3xl p-6 border border-[#2a2a2a]">
            ...
            <div className="mb-4">
              ...
              {amount && (() => {
                ...
                const tokens = inrValue / tokenRate;
                return (
                  <div className="mt-2 space-y-1 bg-[#0a0a0a] p-3 rounded-xl border border-[#2a2a2a]">
                    ...
                    <p className="text-white text-sm font-bold">You will get: {tokens.toFixed(2)} JToken</p>
                  </div>
                );
              })()}
            </div>
