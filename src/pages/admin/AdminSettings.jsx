...
const AdminSettings = () => {
  ...
  const [formData, setFormData] = useState({ usdtRate: 83, tokenRate: 0.01, minJTokenBuy: 10, referralPercent: 5, jTokenCommissionPercent: 4, usdtCommissionPercent: 0, depositCommissionPercent: 0, upiRewardAmount: 50, bankRewardAmount: 100, telegramRewardAmount: 25 });
      ...
      <div className="p-5 space-y-5 max-w-2xl mx-auto">
        ...
        <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] rounded-3xl p-6 border border-[#2a2a2a]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div><label className="block text-gray-400 text-sm mb-2">Deposit Commission (%)</label><input type="number" step="0.1" value={formData.depositCommissionPercent} onChange={(e) => setFormData({ ...formData, depositCommissionPercent: parseFloat(e.target.value) })} className="w-full px-5 py-4 bg-[#0a0a0a] border border-[#2a2a2a] rounded-2xl text-white focus:border-[#D4AF37] focus:outline-none" /></div>
