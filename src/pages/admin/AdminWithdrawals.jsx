    ...
    <div className="min-h-screen bg-[#0a0a0a] pb-24 lg:pb-8 font-['Ubuntu',sans-serif]">
            ...
            <div className="space-y-4">
              <div className="bg-[#0a0a0a] rounded-2xl p-4 border border-[#1a1a1a]">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="text-gray-400">Mobile Verified:</div><div className={userDetails.user?.mobileverified ? 'text-green-400' : 'text-gray-500'}>{userDetails.user?.mobileverified ? '✓ Verified' : 'Not Verified'}</div>
      ...
      <div className={`fixed top-0 left-0 h-full w-80 bg-[#0d0d0d] z-50 transform transition-transform duration-300 lg:hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 space-y-2">
          {menuItems.map((item, index) => (<button key={index} onClick={() => { navigate(item.path); setSidebarOpen(false); }} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-left transition-all ${item.path === '/admin/withdrawals' ? 'bg-[#D4AF37]/10 text-white' : 'bg-[#1a1a1a]/50 hover:bg-[#D4AF37]/10 text-gray-300'}`}><div className={`w-10 h-10 rounded-xl flex items-center justify-center ${item.path === '/admin/withdrawals' ? 'bg-[#D4AF37]/20' : 'bg-[#1a1a1a]'}`}><svg className="w-5 h-5 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} /></svg></div><span className="font-medium">{item.label}</span></button>))}
      ...
      <div className="sticky top-0 z-30 bg-[#0d0d0d]/80 backdrop-blur-2xl border-b border-[#1a1a1a]">
        <div className="flex items-center justify-between gap-2 px-3 py-3 sm:px-4 sm:py-3">
          ...
          <h1 className="min-w-0 flex-1 truncate text-center text-lg font-bold text-white">Withdrawals</h1>
      ...
      <div className="p-4 space-y-4">
                ...
                <div key={w.id} className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-3">
                      <div>
                        <p className="text-white font-medium text-sm">{w.email || 'N/A'}</p>
                        <p className="text-gray-500 text-xs">{formatDate(w.createdat)}</p>
                        <p className="text-gray-400 text-xs mt-1">{w.method || 'Wallet'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <p className="text-gray-500 text-xs font-mono">{w.userid?.slice(0, 8)}...</p>
                      <button 
                        onClick={() => { navigator.clipboard.writeText(w.userid); }}
                      ...
                      <button onClick={() => fetchUserDetails(w.userid)} className="p-2 bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500/30" title="View User">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                      </button>
                      <span className={`px-2 py-1 rounded-lg text-xs font-medium ${w.status === 'APPROVED' ? 'bg-green-500/20 text-green-400' : w.status === 'REJECTED' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>{w.status}</span>
                    </div>
                  </div>
                  <div className="flex justify-between items-center">
                    <p className="text-[#D4AF37] font-bold text-lg">₹{parseFloat(w.amount || 0).toFixed(2)}</p>
                    {w.status === 'PENDING' && (
        ...
        {total > 20 && (
          <div className="flex justify-center gap-2">
            ...
            <button onClick={() => setPage(p => p + 1)} disabled={withdrawals.length < 20} className="px-4 py-2 bg-[#1a1a1a] text-white rounded-xl disabled:opacity-50">Next</button>
          </div>
        )}
      </div>
      <div className="fixed bottom-0 left-0 right-0 bg-[#0d0d0d]/95 backdrop-blur-2xl border-t border-[#1a1a1a] lg:hidden z-50">
        <div className="flex items-center justify-around py-2 px-1">
          ...
          <button onClick={() => navigate('/admin/exchange')} className="flex flex-col items-center gap-1 p-2 text-gray-500"><div className="w-9 h-9 rounded-xl bg-[#1a1a1a] flex items-center justify-center"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg></div><span className="text-xs">Exchange</span></button>
